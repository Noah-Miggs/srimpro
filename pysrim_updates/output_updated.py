...
class SRIM_Output(object):
    ...
    def _read_target(self, output):
        # find text block containing target material information
        match_target = re.search(
            r'=+\s*TARGET MATERIAL.*?^\s*Target energies for target atom.*$',
            output,
        ...
        # store text block containing target material information
        block = match_target.group(0)
        
        # split target material block into sub-blocks, each for one layer of the target material
        layer_blocks = re.split(r'(Layer\s*#.*?-\s*Layer\s*\d+|^=+\s*$)',block,flags=re.MULTILINE)
        
        # loop through layer sub-blocks to get layer depth and density data
        results = []
        for i, layer_text in enumerate(layer_blocks, start=1):
            depth_match = re.search(r'Bottom Depth\s*=\s*([0-9.+\-E]+)',layer_text)
            density_match = re.search(r'Density\s*=\s*([0-9.+\-E]+)\s*atoms/cm3\s*=\s*([0-9.+\-E]+)',layer_text)
        
            if not (depth_match and density_match):
                continue
        
            results.append({
                "layer_index": i,
                "bottom_depth_A": float(depth_match.group(1)),
                "atomic_density": float(density_match.group(1)),
                "mass_density": float(density_match.group(2)),
            })
...
class Collision:
    ...
    def _read_ion(self, ion_str):
        ...
        # empty array for collision data to be added to
        collisions = []
        
        # separate numpy array to store only positions of collisions, for easier access
        collipos = np.empty((0,3))
        
        # separate numpy array to store only positions of PKA collisions (path of ion), for easier access
        ionpos = np.empty((0,3))
        
        # separate numpy array to store PKA energies
        recoilE = np.array([])
        
        # separate numpy array to store number of vacancies caused by each cascade/PKA
        PKA_disps = np.array([])

        # switch lines to an iterable to allow line to be set back to a line that was already read in special cases
        lines = iter(lines)
        
        # variable to store line for special Collision.txt formatting case
        buffered_line = None
        
        # read lines to get ion collision data
        while True:
            try:
                if buffered_line is not None: # if special case encountered, resume reading from buffered_line
                    line = buffered_line
                    buffered_line = None
                else:
                    line = next(lines)
            except StopIteration: # stop loop if it reaches end of text block
                break
            # stop loop at first line of ion footer
            if line.strip().startswith("- For Ion"):
                break

            # Detect cascade start BEFORE parsing tokens
            if "<== Start of New Cascade" in line:
                (target_disp,
                 ...
                 PKApos,
                 PKAenergy,
                 casDisp) = self._read_cascade(lines)
                ...
                collipos = np.append(collipos, caspos, axis=0)
                ionpos = np.append(ionpos, [PKApos], axis=0)
                recoilE = np.append(recoilE, PKAenergy)
                PKA_disps = np.append(PKA_disps, casDisp)
                
                if next_line: # if the special case occured in _read_cascade, go back to next_line
                    buffered_line = next_line
                
                continue

        # search for ion number in the first line of the ion footer
        footer_start = line
        ion_number = re.search(int_regex, footer_start).group(0)
        ...
        def extract_value(label, text):
            match = re.search(label + r"\s*=\s*([0-9.E+-]+)", text)
        ...
        return {
            ...
            'displacements': extract_value("Displacements", footer),
            'avg_displacements': extract_value("Average Displacements/Ion", footer),
            'replacements': extract_value("Replacements", footer),
            'avg_replacements': extract_value("Average Replacements/Ion", footer),
            'vacancies': extract_value("Vacancies", footer),
            'avg_vacancies': extract_value("Average Vacancies/Ion", footer),
            'interstitials': extract_value("Interstitials", footer),
            'avg_interstitials': extract_value("Average Interstitials/Ion", footer),
            'sputtered_atoms': extract_value("Sputtered Atoms", footer),
            'avg_sputtered_atoms': extract_value("Average Sputter Atoms/Ion", footer),
            'transmitted_atoms': extract_value("Transmitted Atoms", footer),
            'avg_transmitted_atoms': extract_value("Average Transmitted Atoms/Ion", footer),
            'collisions': collisions,
            'collision_positions': collipos,
            'PKA_positions': ionpos,
            'PKA_energies': recoilE,
            'PKA_displacements': PKA_disps
        }

    def _read_cascade(self, lines):
        # move to next line (= separator line)
        line = next(lines)

        s = line.strip()
        assert s and set(s) == {'='}
        
        # move two lines down, to first line containing recoil data
        line = next(lines)
        
        # blank array to add cascade data to
        cascade = []
        
        # separate numpy array to store only positions of cascades, for easier access
        caspos = np.empty((0,3))
        
        # separate numpy array to store only position of the first collision within cascade (PKA), for easier access
        PKApos = np.empty((1,3))
        
        # variable to count the number of displacements caused by each cascade
        casDisp = 0
        
        # blank variable to save a line in, for special cases
        next_line = None
        
        # loop through lines to read cascade data
        for line in lines:
            
            # special case: inconsistent formatting of COLLISON.txt
            if "<== Start of New Cascade" in line:
                next_line = line # save line so _read_ion can go back to it
                break
            
            # stop reading lines if separator that indicates end of cascade is encountered
            if set(line.strip()) == {'='}:
                break
            
            clean = re.sub(r"[^\x00-\x7F]+", "", line).strip() # remove all non-ASCII characters from line
            ...
            # safely get data from current line
            recoil_id = int(tokens[0])
            atom = int(tokens[1])
            energy = float(tokens[2])
            x = float(tokens[3]) if len(tokens) > 3 else None
            y = float(tokens[4]) if len(tokens) > 4 else None
            z = float(tokens[5]) if len(tokens) > 5 else None
            vac = int(tokens[6]) if len(tokens) > 6 else 0
            repl = int(tokens[7]) if len(tokens) > 7 else 0
            
            # add data from current line to cascade
            try:
                cascade.append({
                    'recoil': recoil_id,
                    'atom': atom,
                    'recoil_energy': energy,
                    'position': np.array([x,y,z]),
                    'vac': vac,
                    'repl': repl})
                caspos = np.append(caspos, [[x,y,z]], axis=0)
                casDisp = casDisp + vac + repl
            except ValueError:
                # Skip lines like headers or weird formatting
                continue
        
        # store PKA coordinates and energy
        PKApos = caspos[0]
        PKAenergy = cascade[0]['recoil_energy']
        
        if line.count('=') > 100:
            return None, None, None, None, cascade, next_line, caspos, PKApos, PKAenergy, casDisp
        
        # read cascade summary line only if special case not encountered
        if next_line is None:
            ...
            try:
                if len(tokens) >= 5:
                    target_disp = float(tokens[1])
                    target_vac = float(tokens[2])
                    target_replac = float(tokens[3])
                    target_inter = float(tokens[4])
                else:
                    raise ValueError
            except (ValueError, IndexError):
                target_disp = None
        ...
        return target_disp, target_vac, target_replac, target_inter, cascade, next_line, caspos, PKApos, PKAenergy, casDisp
