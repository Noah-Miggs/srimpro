...
def recoilSpectra(paths, labels, n=100, num_ions=None, ion_start=0, minE=None, maxE=None, graph_style='line', 
    ...
    minE : scalar or None, default=None
        minumum energy in eV (defaults to 1 eV)
    maxE : scalar or None, default=None
        maximum energy in eV (defaults to maximum collision energy from SRIM)
    ...
    if excelpath is not None:
        ...
        # restrict excel data to only include data for PKA energies above 1 eV
        subset = (x_energy >= 1)
...
def weightedRecoilSpectra(paths, labels, n=100, num_ions=None, ion_start=0, minE=None, maxE=None, graph_style='line', 
    ...
    try:
        ...
        for i in range(len(paths)):
            ...
            for j in range(n):
                ...
                # for the first energy bin, manually sum the number of recoils and displacements at the minimum energy 
                # since these recoils are skipped by the logic above (for the total spectra option)
                if j == 0 and spectra == 'total':
                    subset = (rec_energies[i] == energy[j])
                    energy_recs = rec_energies[i][subset]
                    disp_recs = rec_disps[i][subset]
        ...
        # multiply number of recoils and displacements by each recoil energy together and normalize by the total number 
        # of recoils an ddisplacements for that path to get the weighted recoil spectra
        for i in range(len(paths)):
    ...
    if excelpath is not None:
        ...
        # restrict excel data to only include data above PKA energies above 1 eV
        subset = (x_energy >= 1)
        x_energy = x_energy[subset]
        x_spectrum = x_spectrum[subset]
