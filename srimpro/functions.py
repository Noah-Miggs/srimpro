...
# angstrom.
def totDisplacements(path, x1=None, x2=None):
    # docstring
    """
    Calculates the total number of vacancies and replacements.
    
    Parameters
    ----------
    path : string
        file path to folder containing SRIM output files
    x1: scalar or None, default=None
        minimum depth in angstroms (defaults to 0)
    x2: scalar or None, default=None
        maximum depth in angstroms (defaults to maximum depth from SRIM)
    ...
    vactot = simpsons(depth, vac)
    novactot = simpsons(depth, novac)
...
# given in angstrom.
def totEnergyDep(path, x1=None, x2=None, method='cons'):
    # docstring
    """
    Calculates the total ionization and damage energy.
    ...
    x2: scalar or None, default=None
        maximum depth in angstroms (defaults to maximum depth from SRIM)
    method : string, default='cons'
        method to use to calculate total damage energy
        (defaults to energy conservation method)
    
    Returns
    -------
    list1 : object
        2-element list [ionization energy, ionization energy %]
    list2 : object
        2-element list [damage energy, damage energy %]
    """
...
# assumption: path given as a string, energy deposition profiles and depth values in SRIM in eV and angstrom, x1 & x2
# given in angstrom.
def totStopping(path, x1=None, x2=None):
    # docstring
    """
    Calculates the total energy deposited as electronic and nuclear stopping
    ...
    -------
    list1 : object
        2-element list [electronic stopping, electronic stopping %]
    list2 : object
        2-element list [nuclear stopping, nuclear stopping %]
    """
...
# for ion density (either 'at_%' or 'density_per_fluence', defaults to 'at_%'), ion_range = indicator for plotting ion 
# range profile (on by default), dose = indicator for plotting damage dose profile (on by default), layerdepth = array of 
# bottom depths of target material layers (if not specified, reads this info from TDATA.txt), output = indicator for what
# to output (either 'plot' or 'excel', defaults to 'plot').
...
def rangeAndDpa(path, fluence, x1=None, x2=None, units='at_%', ion_range=True, dose=True, layerdepth=None, output='plot'):
    # docstring
    """
    Creates ion distribution and damage dose plot or exports data to excel.
    ...
    path : string
        file path to folder containing SRIM output files
    fluence : scalar
        irradiation fluence in cm^-2
    x1 : scalar or None, default=None
        minimum depth in angstroms (defaults to 0)
    x2 : scalar or None, default=None
        maximum depth in angstroms (defaults to maxmimum depth from SRIM)
    units : string, default='at_%'
        units to use for ion distribution (defaults to [at. %])
    ion_range : bool, default=True
        option for plotting ion range profile
    dose : bool, default=True
        option for plotting damage dose profile
    layerdepth : object or None, default=None
        list of depth of each layer (defaults to values in SRIM output files)
    output : string, default='plot'
        output option to use (defaults to creating plot)
    """
    ...
    # check that input for ionrange is valid
    if not (ion_range == True or ion_range == False):
        raise ValueError("Only ionrange True or False is accepted")
    
    # check that input for dpa is valid
    if not (dose == True or dose == False):
        raise ValueError("Only dpa True or False is accepted")
    
    # check that at least one plotting option enabled
    if not (ion_range == True or dose == True):
        raise ValueError("At least one plotting option (ion range or damage dose) must be enabled")
    ...
    ionrange = Range.ions # ion range distribution
    ...
    if output == 'plot':
        ...
        # if one plotting option disabled
        if ion_range == False or dose == False:
            if ion_range: # if ion range option enabled
            ...
            elif dose: # if damage dose option enabled
                ln1 = ax1.plot(depth, dpa, color='purple', linewidth=linewidth, linestyle=linestyle, marker=marker2, 
            ...
            plt.show()
        # if both plotting options enabled
        else:
            # ion range plotting
            ln1 = ax1.plot(depth, ionrange, color='blue', linewidth=linewidth, linestyle=linestyle, marker=marker1, 
                           markersize=markersize, label="Ion Distribution")
...
# ionizRec = indicator for ionization energy by recoil, total = indicator for total energy deposition, method = indicator 
# for what method to use for calculation of damage energy (either 'phonons_1' which calculates damage energy by adding 
# phonons by recoils and phonons by ion from PHONON.txt file, 'phonons_2' which calculates damage energy by using only 
# phonons by recoils from PHONON.txt file, or 'cons' which calculates damage energy profile by multiplying each layer of 
# the total displacement profile by a local Tdam/v factor specific to that layer, where Tdam is the damage energy 
# calculated as incident ion energy deposited in the layer minus the total ionization energy deposited in the layer, and v 
# is the total displacements in the layer), layerdepth = array of bottom depths of target material layers (if not 
# specified, reads this info from TDATA.txt), scale = indicator for what scale to use for first y-axis (either 'log' or 
...
def energyDep(path, x1=None, x2=None, damage=True, ionizTot=True, ionizIon=True, ionizRec=True, total=True, method='cons',
              layerdepth=None, scale='linear', output='plot'):
    # docstring
    """
    Creates energy deposition plot or exports data to excel.
    ...
    x2 : scalar or None, default=None
        maximum depth in angstroms (defaults to maxmimum depth from SRIM)
    damage : bool, default=True
        option for plotting damage energy (on by default)
    ionizTot : bool, default=True
        option for plotting total ionization energy (on by default)
    ionizIon : bool, default=True
        option for plotting ionization energy by ion (on by default)
    ionizRec : bool, default=True
        option for plotting ionization energy by recoils (on by default)
    total : bool, default=True
        option for plotting total energy (on by default)
    method : string, default='cons'
        method to use to calculate total damage energy
        (defaults to energy conservation method)
    layerdepth : object or None, default=None
    ...
    scale : string, default='linear'
        scale to use for y-axis (defaults to linear)
    output : string, default='plot'
        output option to use (defaults to creating plot)
    """
    ...
    # check that input for total is valid
    if not (total == True or total == False):
        raise ValueError("Only total True or False is accepted")
    ...
    if method == 'phonons_1':
        ...
        vactot, novactot = totDisplacements(path, x1=0, x2=layerdepth[0]*10)
        v = vactot + novactot # total number of displacements
        ionizE, damE = totEnergyDep(path, x1=0, x2=layerdepth[0]*10, method='cons')
        ...
        for i in range(1,len(layerdepth)):
            ...
            vactot, novactot = totDisplacements(path, x1=layerdepth[i-1]*10, x2=layerdepth[i]*10)
            v = vactot + novactot # total number of displacements
            ionizE, damE = totEnergyDep(path, x1=layerdepth[i-1]*10, x2=layerdepth[i]*10, method='cons')
    ...
    # calculate total energy profile
    tot = ioniztot + damtot
    ...
    color5 = 'orange' # colour for total energy
    ...
    marker5 = 'X' # marker for total energy
    ...
    if output == 'plot':
        ...
        if total: # if total energy option selected
            ax1.plot(depth, tot, color=color5, linewidth=linewidth, linestyle=linestyle, marker=marker5, 
                       markersize=markersize, label="Total Energy")
        ax1.xaxis.set_minor_locator(AutoMinorLocator())
        ax1.yaxis.set_minor_locator(AutoMinorLocator())
        ax1.tick_params(axis='both', which='major', length=majorlength, width=tickwidth, labelsize=ticklabsize, 
                        direction='in')
...
def stopping(path, x1=None, x2=None, ratio=None, scale='linear', output='plot'):
    # docstring
    """
    Creates stopping power plot or exports data to excel.
    ...
    x2 : scalar or None, default=None
        maximum depth in angstroms (defaults to maxmimum depth from SRIM)
    ratio : string or None, default=None
        what stopping power ratio to plot (defaults to no ratio)
    scale : string, default='linear'
        scale to use for y-axis (defaults to linear)
    output : string, default='plot'
        output option to use (defaults to creating plot)
...
def TdamCompare(path, x1=None, x2=None, layerdepth=None, output='plot'):
    # docstring
    """
    Creates damage energy method comparison plot or exports data to excel.
    ...
    ionizE, damphon1_tot = totEnergyDep(path, x1=x1*10, x2=x2*10, method='phonons_1')
    ionizE, damphon2_tot = totEnergyDep(path, x1=x1*10, x2=x2*10, method='phonons_2')
    ionizE, damcons_tot = totEnergyDep(path, x1=x1*10, x2=x2*10, method='cons')
...
# integers.
def collisionPlotDepth(path, x1=None, x2=None, n1=100, d2='z', r=None, n2=None, num_ions=None, ion_start=0, coll_type='ions', 
                       scale='linear'):
    # docstring
    """
    Creates 2D contour plot of the collision distribution "side-view".
    ...
    x2: scalar or None, default=None
        maximum depth in angstroms (defaults to maximum depth from SRIM)
    n1 : int, default=100
        number of x bins (defaults to 100)
    d2 : string, default='z'
        second dimension to plot (defaults to z)
    r : scalar or None, default=None
        y/z "radius" (defaults to maximum collision radius from SRIM)
    n2 : int or None, default=None
        number of y/z bins (defaults to the same value as n1)
    num_ions : int or None, default=None
        number of ions to plot (defaults to all calculated ions)
    ion_start : int, default=0
        index of first ion to plot (defaults to first ion calculated)
    coll_type : string, default='ions'
        type of collisions to plot (defaults to only ion-target collisions)
    scale : string, default='linear'
        colour bar scale to use (defaults to linear)
    """
    ...
    # set the maximum and minimum depth
    if x1 is None:
        x1 = 0
    else:
        x1 = x1*0.1 # convert to nm from angstroms
    if x2 is None:
        x2 = depth[-1]
    else:
        x2 = x2*0.1 # convert to nm from angstroms
...
# angstroms, r is a scalar and is given in units of angstroms, n1 & n2 are integers, ion_start and num_ions are given as
# integers.
def collisionPlotXsection(path, x1=None, x2=None, n=100, r=None, num_ions=None, ion_start=0, coll_type='ions', scale='linear'):
    # docstring
    """
    Creates 2D contour plot of the collision distribution "front-view".
    ...
    x2: scalar or None, default=None
        maximum depth in angstroms (defaults to maximum depth from SRIM)
    n : int, default=100
        number of y and z bins (defaults to 100)
    r : scalar or None, default=None
        y and z "radius" (defaults to maximum collision radius from SRIM)
    num_ions : int or None, default=None
        number of ions to plot (defaults to all calculated ions)
    ion_start : int, default=0
        index of first ion to plot (defaults to first ion calculated)
...
# where the elements are strings corresponding to the labels for the data in paths, fluences = 1D array where elements are 
# the ion fluences corresponding to each path in paths, x1 = minimum depth value, x2 = maximum depth value, 
# units = indicator for what units to use for ion density (either 'at_%' or 'density_per_fluence', defaults to 'at_%'), 
# plot_style = indicator for plotting the ion distribution and damage dose profiles as single combined profiles (by summing
# the individual profiles), ref_layer = what layer to use as the reference (zero) for the depth axis (defaults to the first
# layer), layerdepth = an object array where each element corresponds to a path and is an array with the bottom depths of 
# each target material layer for that path.
...
def multiRangeAndDpa(paths, labels, fluences, x1=None, x2=None, units='at_%', plot_style='individual', ref_layer=1, 
                     layerdepth=None):
    # docstring
    """
    Creates an ion distribution and a damage dose plot for multiple SRIM sims.
    
    Parameters
    ----------
    paths : object
        list of file paths to folders containing SRIM output files
    labels : object
        list of labels corresponding to each file path in paths
    fluences : object
        list of irradiation fluences in cm^-2 for each path in paths
    x1: scalar
        minimum depth in angstroms
    x2: scalar
        maximum depth in angstroms
    units : string, default='at_%'
        units to use for ion distribution (defaults to [at. %])
    plot_style : string, default='individual'
        option for how to show data from different simulations
        (defaults to not combining profiles)
    ref_layer : int, default=1
        layer to use as the reference layer (defaults to first layer)
    layerdepth : object or None, default=None
        object array where each element is a list of the layer depths for a
        simulation (defaults to values in SRIM output files)
    """
    ...
    # empty array to add range data to, rows correspond to paths, columns correspond to ion distribution at each depth
    ...
    for i in range(len(paths)):
        ...
        # access the required data within output files
        ionrange[i] = Range.ions # ion range distribution
        ...
        if len(layerdensity[i]) < 2:
            dpa[i][:layerindex[0]+1] = displacements[:layerindex[0]+1]*fluences[i]/layerdensity[i]*1e8
        else:
            dpa[i][:layerindex[0]+1] = displacements[:layerindex[0]+1]*fluences[i]/layerdensity[i][0]*1e8 # first layer
            for j in range(layerindex.size-1): # loop through all subsequent target layers to calculate the dpa in each
                dpa[i][layerindex[j]+1:layerindex[j+1]+1] = (displacements[layerindex[j]+1:layerindex[j+1]+1]
                                                          *fluences[i]/layerdensity[i][j+1]*1e8)
        ...
        if units == 'at_%':
            ...
            if len(layerdensity[i]) < 2:
                ...
                ionrange[i][:layerindex[0]+1] = ionrange[i][:layerindex[0]+1]*fluences[i]*100/layerdensity[i]
            else:
                # convert ionrange data from ion density to atomic percent, accounting for different layer densities
                ionrange[i][:layerindex[0]+1] = ionrange[i][:layerindex[0]+1]*fluences[i]*100/layerdensity[i][0]
                for j in range(layerindex.size-1): # loop through all subsequent target layers
                    ionrange[i][layerindex[j]+1:layerindex[j+1]+1] = (ionrange[i][layerindex[j]+1:layerindex[j+1]+1]
                                                              *fluences[i]*100/layerdensity[i][j+1])
...
# file with additional data to plot (defaults to not plotting additional data), output = indicator for what to output 
# (either 'plot' or 'excel', defaults to 'plot').
...
def recoilSpectra(paths, labels, n=100, num_ions=None, ion_start=0, minE=None, maxE=None, graph_style='line', 
                  xscale='log', yscale='linear', spectra='cumulative', excelpath=None, output='plot', x1=None,
                  x2=None):
    # docstring
    """
    Creates primary recoil spectra plot for multiple sims or exports to excel.
    ...
    labels : object
        list of labels corresponding to each file path in paths
    n : int, default=100
        number of energy bins to use (defaults to 100)
    num_ions : int or None, default=None
        number of ions to plot (defaults to all calculated ions)
    ion_start : int, default=0
        index of first ion to plot (defaults to first ion calculated)
    minE : scalar or None, default=None
        minumum energy in eV (defaults to 10 eV)
    maxE : scalar or None, default=None
        maximum energy in eV (defaults to maximum collision energy from all paths)
    graph_style : string, default='line'
        style of graph to create (defaults to regular line plot)
    xscale : string, default='log'
        scale to use for x-axis and energy bins (defaults to log base 10)
    yscale : string, default='linear'
        scale to use for y-axis (defaults to linear)
    spectra : string, default='cumulative'
        type of primary recoil spectra to calculate (defaults to cumulative)
    excelpath : string or None, default=None
        file path to an additional excel file with data to plot
        (defaults to not including additional data)
    output : string, default='plot'
        output option to use (defaults to creating plot)
    x1: scalar or None, default=None
        minimum depth in angstroms (defaults to 0)
    x2: scalar or None, default=None
        maximum depth in angstroms (defaults to maximum depth from all paths)
    """
    ...
    # check that inputs for x1 and x2 are valid
    ...
    # variable to store what type of files were given in paths
    filetype = 'SRIM'
    
    # convert units of minumum and maximum depth if specified (convert to nm from angstroms)
    if x1 is not None:
        x1 = x1*0.1
    if x2 is not None:
        x2 = x2*0.1
    
    # read data from COLLISON.txt and perform calculations if direct SRIM output files given in paths
    try:
        ...
        for i in range(len(paths)):
            ...
            # empty array to add recoil depth data to
            recX = np.array([])
            ...
            for j in range(ion_start, num_ions + ion_start):
                try:
                    ...
                    if (x1 is not None) or (x2 is not None):
                        recX = np.append(recX, coll[j]['PKA_positions'][:,0])
                    ions_used = ions_used + 1
                except (IndexError, AttributeError):
                    break
            ...
            # restrict data to only that within the specified depth range (if a depth range was specified)
            if (x1 is not None) or (x2 is not None):
                recX = recX*0.1 # convert to nm from angstroms
                subset = ((recX >= x1) & (recX <= x2))
                recE = recE[subset]
        ...
        for i in range(len(paths)):
            ...
            for j in range(n):
                ...
                if spectra == 'total':
                    ...
                    subset = ((rec_energies[i] <= energy[j+1]) & (rec_energies[i] >= 10))
        ...
        # remove first element of energy so plot will only use high energy bounds
        energy = energy[1:]
    
    # if excel files given within paths, read them
    except (FileNotFoundError):
        # variable to store what type of files were given in paths
        filetype = 'excel'
        
        # empty array to store number of recoils with each energy for each path
        energy = np.empty((len(paths)), dtype=object)
        
        # empty array to store number of recoils with each energy for each path
        numRec = np.zeros((len(paths)), dtype=object)
        
        # read data from each excel file
        for i in range(len(paths)):
            # energy data
            pathE = pd.read_excel(paths[i], usecols=[0], skiprows=0, header=None).squeeze()
            pathE = pd.to_numeric(pathE, errors='coerce')
            pathE = np.array(pathE.dropna())
            energy[i] = pathE
            
            pathRec = pd.read_excel(paths[i], usecols=[1], skiprows=0, header=None).squeeze()
            pathRec = pd.to_numeric(pathRec, errors='coerce')
            pathRec = np.array(pathRec.dropna())
            numRec[i] = pathRec
    ...
    if excelpath is not None:
        ...
        x_widths = np.diff(x_energy)*-1
        
        # restrict excel data to only include data for PKA energies above 10 eV
        subset = (x_energy >= 10)
    ...
    if output == 'plot':
        ...
        for i in range(len(paths)):
            if graph_style == 'bar':
                if filetype == 'SRIM':
                    ax1.bar(energy, numRec[i], width=widths, label=labels[i], alpha=opacity, align='edge')
                elif filetype == 'excel':
                    ax1.bar(energy[i], numRec[i], width=widths, label=labels[i], alpha=opacity, align='edge')
            elif graph_style == 'line':
                if filetype == 'SRIM':
                    ax1.plot(energy, numRec[i], label=labels[i], linewidth=linewidth, linestyle=linestyle)
                elif filetype == 'excel':
                    ax1.plot(energy[i], numRec[i], label=labels[i], linewidth=linewidth, linestyle=linestyle)
        if excelpath is not None:
            if graph_style == 'bar':
        ...
        if yscale == 'log':
            ax1.set_yscale('log')
        # add legend and show plot
        plt.legend(fontsize=legendlabsize, loc='lower right', bbox_to_anchor=(1.425, 0))
        plt.show()
    
    # export data to excel if excel option selected
    elif output == 'excel':
        # create excel file for each path in paths
        for i in range(len(paths)):
            # convert arrays to a pandas data frame so it can be exported to excel
            data = np.empty((len(numRec[i]),2))
            data[:,0] = energy
            data[:,1] = numRec[i]
            df = pd.DataFrame(data, columns=["PKA energy upper bound (eV)", f"{spectra} recoil fraction"])
           
            # in case function has run multiple times for same path, rename file to be created accordingly
            num = 0
            while True:
                # create filepath to export data frame to
                export_path = os.path.join(paths[i], f"({labels[i]})_PKA_spectrum({num}).xlsx")
...
# spectra to calculate and plot (either 'cumulative' or 'total', defaults to 'cumulative'), excelpath = path to an excel
# file with additional data to plot (defaults to not plotting additional data), output = indicator for what to output 
# (either 'plot' or 'excel', defaults to 'plot'), excel_format = format used in the excelpath excel file (either 1 or 2,
# defaults to 1), x1 = minimum depth, x2 = maximum depth.
# assumptions: path given as a string, COLLISON.txt file within 'path' directory (option must be selected before SRIM
...
def weightedRecoilSpectra(paths, labels, n=100, num_ions=None, ion_start=0, minE=None, maxE=None, graph_style='line', 
                          xscale='log', yscale='linear', spectra='cumulative', excelpath=None, output='plot', 
                          excel_format=1, x1=None, x2=None):
    # docstring
    """
    Creates weighted primary recoil spectra plot for multiple sims or exports to excel.
    ...
    yscale : string, default='linear'
        scale to use for y-axis (defaults to linear)
    spectra : string, default='cumulative'
        type of weighted primary recoil spectra to calculate
    ...
    output : string, default='plot'
        output option to use (defaults to creating plot)
    excel_format : int, default=1
        format used in the additional excel file at excelpath
    ...
    # check that input for excel_format is valid
    if not (excel_format == 1 or excel_format == 2):
        raise ValueError("Only excel_path 1 or 2 is accepted")
    ...
    try:
        ...
        for i in range(len(paths)):
            ...
            # store recoil energies and displacements for the current path in rec_energies and rec_disps
            rec_energies[i] = recE
            rec_disps[i] = recD
        ...
        for i in range(len(paths)):
            # loop through the energy bins and count the number of recoils and displacements that 
            # occurred within each
            for j in range(n):
                if spectra == 'total':
                    subset = ((rec_energies[i] > energy[j]) & (rec_energies[i] <= energy[j+1]))
                elif spectra == 'cumulative':
                ...
                # for the first energy bin, manually sum the number of recoils and displacements at the
                # minimum energy since these recoils are skipped by the logic above (for the total 
                # spectra option)
                if j == 0 and spectra == 'total':
                    subset = (rec_energies[i] == energy[j])
                    energy_recs = rec_energies[i][subset]
        ...
        # multiply number of recoils and displacements by each recoil energy together and normalize by 
        # the total number of recoils an ddisplacements for that path to get the weighted recoil spectra
        for i in range(len(paths)):
    ...
    # if excel files given within paths, read them
        ...
        # empty array to store number of recoils with each energy for each path
        weightedSpectra = np.empty((len(paths)), dtype=object)
        ...
        for i in range(len(paths)):
            ...
            pathRec = pd.to_numeric(pathRec, errors='coerce')
            pathRec = np.array(pathRec.dropna())
            weightedSpectra[i] = pathRec
    
    # read additional excel file if specified
    if excelpath is not None:
        x_energy = pd.read_excel(excelpath, usecols=[0], skiprows=0, header=None).squeeze()
        x_energy = pd.to_numeric(x_energy, errors='coerce')
        x_energy = np.array(x_energy.dropna())
        ...
        if excel_format == 2:
            ...
            totdpa = np.sum(x_dpa)
            if spectra == 'cumulative':
                for i in range(len(x_energy)):
                    x_spectrum[i] = x_spectrum[i]*np.sum(x_dpa[:i+1])/totdpa
            elif spectra == 'total':
                x_spectrum = x_spectrum*x_dpa/totdpa
        
        # calculate widths to use for bar graph
        x_widths = np.diff(x_energy)*-1
        
        # restrict excel data to only include data above PKA energies above 10 eV
        subset = (x_energy >= 10)
        x_energy = x_energy[subset]
        x_spectrum = x_spectrum[subset]
    ...
    if output == 'plot':
        ...
        for i in range(len(paths)):
            if graph_style == 'bar':
                if filetype == 'SRIM':
                    ax1.bar(energy, weightedSpectra[i], width=widths, label=labels[i], alpha=opacity, align='edge')
                elif filetype == 'excel':
                    ax1.bar(energy[i], weightedSpectra[i], width=widths, label=labels[i], alpha=opacity, align='edge')
            elif graph_style == 'line':
                if filetype == 'SRIM':
                    ax1.plot(energy, weightedSpectra[i], linewidth=linewidth, linestyle=linestyle, label=labels[i])
                elif filetype == 'excel':
                    ax1.plot(energy[i], weightedSpectra[i], linewidth=linewidth, linestyle=linestyle, label=labels[i])
        if excelpath is not None:
            if graph_style == 'bar':
                ax1.bar(x_energy, x_spectrum, width=x_widths, label=labels[-1], alpha=opacity, align='edge')
            elif graph_style == 'line':
    ...
    # export data to excel if excel option selected
        ...
        for i in range(len(paths)):
            # convert arrays to a pandas data frame so it can be exported to excel
            data = np.empty((len(weightedSpectra[i]),2))
            data[:,0] = energy
            data[:,1] = weightedSpectra[i]
            df = pd.DataFrame(data, columns=["PKA energy upper bound (eV)", f"{spectra} weighted recoil fraction"])
           
            # in case function has run multiple times for same path, rename file to be created accordingly
            num = 0
            while True:
                # create filepath to export data frame to
                export_path = os.path.join(paths[i], f"({labels[i]})_weighted_PKA_spectrum({num}).xlsx")
